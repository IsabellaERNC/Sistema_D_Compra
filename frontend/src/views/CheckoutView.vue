...
onMounted(() => {
  if (!auth.isLoggedIn) {
    ...
    if (!import.meta.env.VITE_AUTH_URL) {
      console.error('[CheckoutView] VITE_AUTH_URL not configured — cannot redirect to auth')
      showToast('Error de configuración: Servicio de autenticación no disponible', 'error', 8000)
      return
    }
