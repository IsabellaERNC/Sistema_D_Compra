...
module.exports = (pool, verificarToken, productosClient, io) => {
        ...
        try {
            ...
            const itemsConPrecio = await Promise.all(
                items.map(async (item) => {
                    try {
                        const producto = await productosClient.getProducto(item.producto_id, token);
    ...
    const agregarHandler = async (req, res) => {
        ...
        try {
            let stockInfo;
            try {
                stockInfo = await productosClient.verificarStock(producto_id, token);
            } catch (stockErr) {
    ...
    router.patch('/:producto_id', verificarToken, async (req, res) => {
        ...
        try {
            ...
            if (cant > currentCant) {
                const stockInfo = await productosClient.verificarStock(productoId, token);
    ...
    router.get('/datos-pago', verificarToken, async (req, res) => {
        ...
        try {
            ...
            const items = await Promise.all(
                rows.map(async (r) => {
                    try {
                        const producto = await productosClient.getProducto(r.producto_id, token);
                        const precioActual = parseFloat(producto.precio);
