...
const transaccionesRouter = require('./router/transacciones');
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        if (existe.rows.length > 0) return res.status(409).json({ error: 'Este email ya está registrado.' });
        ...
        const token = jwt.sign({ id: usuario.id, nombre: usuario.nombre, email: usuario.email }, JWT_SECRET, { expiresIn: '24h' });
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        const resultado = await pool.query('SELECT id, nombre, email, password_hash FROM usuarios WHERE email = $1', [email.toLowerCase().trim()]);
        ...
        const token = jwt.sign({ id: usuario.id, nombre: usuario.nombre, email: usuario.email }, JWT_SECRET, { expiresIn: '24h' });
        res.json({ mensaje: `Bienvenido, ${usuario.nombre}!`, token, usuario });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Error interno.' });
    }
...
app.get('/api/auth/me', verificarToken, async (req, res) => {
    try {
        const resultado = await pool.query('SELECT id, nombre, email, created_at FROM usuarios WHERE id = $1', [req.usuario.id]);
...
app.listen(PORT, () => {
    ...
    console.log(`   ANY    /api/transacciones`); 
